AjaxInterceptor = (function (exports) {
    'use strict';
...
(function () {
    ...
    function mungePath(url) {
        var matchedUrl = apiUrl.exec(url);
        return matchedUrl !== null ? matchedUrl[4].replace(/\/*\?/, '_').replace('&', '_').replace('/', '-').replace(/(?:\s+)/g, '') : url;
        //return ((matchedUrl[1]) ? (matchedUrl[2] + '://' + matchedUrl[3]) : '') + '/api/fake/' + matchedUrl[4];
    }

    function saveResponse(store, xhr) {
        var key = mungePath(xhr.requestURL);
        store[key] = JSON.parse(xhr.responseText);
        localStorage.setItem('ajax-siphon', JSON.stringify(store));
        return key;
    }

    function openWindow(key, xhr) {
        var win = window.open('', key,'resizable=true,width=600,height=400');
        win.document.open();
        win.document.writeln(xhr.responseText);
        win.document.close();
        win.document.title = key + '.json';
    }

    function main() {
        var store  = initStore();
        AjaxInterceptor.addResponseCallback(function(xhr) {
           var key = saveResponse(store, xhr);
            openWindow(key, xhr);
        });
